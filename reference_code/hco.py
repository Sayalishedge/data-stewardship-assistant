...
def render_enrichment_page(session, selected_hco_df):
    ...
    if st.session_state.get('show_primary_confirm_dialog'):
        ...
        with dialog_placeholder.container():
            ...
            # AI-generated if: ID starts with 'ai_generated_' OR SOURCE contains 'AI' or 'Generated'
            source_val = str(new_outlet_data.get('SOURCE', '')).upper()
            is_ai_generated = (str(new_primary_id).startswith('ai_generated_') or
                               'GENERATED' in source_val or
                               'AI' in source_val or
                               source_val == 'GENERATED BY AI')
            ...
            with col1:
                if st.button(confirm_btn_text, key="confirm_primary_yes"):
                    ...
                    # Use same robust check as display logic
                    source_val_btn = str(new_outlet_data.get('SOURCE', '')).upper()
                    is_ai_generated = (str(new_primary_id).startswith('ai_generated_') or
                                      'GENERATED' in source_val_btn or
                                      'AI' in source_val_btn or
                                      source_val_btn == 'GENERATED BY AI')
