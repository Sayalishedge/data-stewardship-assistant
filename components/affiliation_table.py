...
def render_affiliation_expander(
    ...
    with st.expander(expander_title, expanded=False):
        ...
        if not priority_rankings:
            ...
            with col2:
                if st.button("🎯 Analyze Priority Order with AI", key=f"analyze_priorities_{record_id}", use_container_width=True):
            ...
            if st.session_state.get(f'analyze_priorities_clicked_{full_cache_key}', False):
                ...
                with st.spinner("⏳ Fetching priority rankings and reasons from OpenAI..."):
